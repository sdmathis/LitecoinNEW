...
uint256 hashGenesisBlock("0x98c46c5f49ebbd535265660e486fe78ac061eb7c7eb3a75f7448ba514d8e4147");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LitecoinNEW: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LitecoinNEW Signed Message:\n";
...
{
    // LitecoinNEW: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LitecoinNEW: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LitecoinNEW: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LitecoinNEW: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x98c46c5f49ebbd535265660e486fe78ac061eb7c7eb3a75f7448ba514d8e4147");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1418839654;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085101785;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitecoinNEWMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitecoinNEWMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running LitecoinNEW with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitecoinNEWMiner terminated\n");
        throw;
    }
}
